#!/usr/bin/env python3
"""Super Metroid configuration editor."""
...
INI_PATH = installer.game_dir({"folder": "SuperMetroid", "console": "Super Nintendo"}, "macOS") / "sm.ini"
...
SETTINGS_META = {
    "General": [
        ...
        ("DisplayPerfInTitle",  "Display performance metrics in title bar",       "bool",   None),
    ],
    "Graphics": [
    ...
    "Sound": [
        ...
        ("EnableMSU",      "MSU-1 music replacement",             "choice", ["0", "1", "opuz", "deluxe", "deluxe-opuz"]),
...
OTHER_KEYMAP = [
    ...
    ("DisplayPerf",      "Toggle performance display"),
    ("ToggleRenderer",   "Toggle renderer"),
    ("ToggleWhichFrame", "Toggle which frame is shown"),
]
...
def _qt_key_to_sm(key: int, modifiers) -> str:
    """Convert a Qt key+modifiers combo to sm key name format."""
    mod_parts = []
...
class KeyCaptureButton(QPushButton):
    ...
    def keyPressEvent(self, event):
        ...
        result = _qt_key_to_sm(event.key(), event.modifiers())
        if result:
            self._value = result
        self._capturing = False
        self._update_text()
...
class KeyboardTab(QScrollArea):
    def __init__(self, ini: IniFile):
        ...
        # ── Reference slots (13 entries each)
...
class ConfigWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Super Metroid Configuration")
        self.setMinimumSize(640, 560)

        if not INI_PATH.exists():
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(None, "Not installed", "Super Metroid is not installed yet.")
            return
