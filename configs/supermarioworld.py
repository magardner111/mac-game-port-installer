#!/usr/bin/env python3
"""Super Mario World configuration editor."""
...
INI_PATH = installer.game_dir({"folder": "SuperMarioWorld", "console": "Super Nintendo"}, "macOS") / "smw.ini"
...
SETTINGS_META = {
    "General": [
        ...
        ("SavePlaythrough",   "Save snapshot each time a level is completed", "bool", None),
    ],
    "Graphics": [
        ("WindowSize",        "Window size (Auto or WxH)",                   "str",    None),
        ...
        ("OutputMethod",      "Render output method",                         "choice", ["SDL", "SDL-Software", "OpenGL"]),
    ...
    "Sound": [
        ...
        ("AudioFreq",     "Sample rate (Hz)",            "choice", ["32000", "44100", "48000", "22050", "11025"]),
        ("AudioChannels", "Channels",                   "choice", ["2", "1"]),
        ("AudioSamples",  "Buffer size (samples)",       "choice", ["512", "1024", "2048", "4096", "256"]),
    ],
}

OTHER_KEYMAP = [
    ("CheatLife",    "Cheat: Extra life"),
    ("CheatJump",    "Cheat: Super jump"),
    ...
    ("Turbo",        "Turbo / fast-forward"),
    ("ReplayTurbo",  "Turbo during replay"),
...
REF_SLOTS_KEYMAP = [
    ("LoadRef",   "Load reference state"),
    ("ReplayRef", "Replay reference state"),
]
...
def _qt_key_to_smw(key: int, modifiers) -> str:
    mod_parts = []
...
class KeyCaptureButton(QPushButton):
    ...
    def keyPressEvent(self, event):
        ...
        result = _qt_key_to_smw(event.key(), event.modifiers())
        if result:
            self._value = result
        self._capturing = False
        self._update_text()
...
class KeyboardTab(QScrollArea):
    def __init__(self, ini: IniFile):
        super().__init__()
        self._ini = ini
        self._button_captures: list[KeyCaptureButton] = []
        ...
        # ── SNES button → key (Player 1)
        group1 = QGroupBox("SNES Button Mapping — Player 1")
        ...
        # ── Reference slots
        group4 = QGroupBox("Reference slots  (keys 0–9, -, =, Backspace)")
        g4_layout = QVBoxLayout(group4)
        self._ref_table = QTableWidget(13, 2)
        self._ref_table.setHorizontalHeaderLabels(["Load Ref", "Replay Ref"])
        self._ref_table.verticalHeader().setVisible(True)
        self._ref_table.setVerticalHeaderLabels([f"Slot {i+1}" for i in range(13)])
        self._ref_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self._ref_table.setFixedHeight(320)
        for col_i, (key, _) in enumerate(REF_SLOTS_KEYMAP):
            raw = ini.get("KeyMap", key)
            slots = [s.strip() for s in raw.split(",")]
            while len(slots) < 13:
                slots.append("")
            for row_i in range(13):
                self._ref_table.setItem(row_i, col_i, QTableWidgetItem(slots[row_i]))
        g4_layout.addWidget(self._ref_table)
    ...
    def apply(self):
        ...
        for col_i, (key, _) in enumerate(REF_SLOTS_KEYMAP):
            slots = []
            for row_i in range(13):
                item = self._ref_table.item(row_i, col_i)
                slots.append(item.text() if item else "")
            self._ini.set("KeyMap", key, ", ".join(slots))
...
class GamepadTab(QScrollArea):
    def __init__(self, ini: IniFile):
        super().__init__()
        self._ini = ini
        self._combos_p1: list[QComboBox] = []
        self._combos_p2: list[QComboBox] = []
        ...
        enable_row = QFormLayout()
        self._enable_p1 = QCheckBox()
        self._enable_p1.setChecked(ini.get("GamepadMap", "EnableGamepad1", "true") in ("1", "true", "yes"))
        self._enable_p2 = QCheckBox()
        self._enable_p2.setChecked(ini.get("GamepadMap", "EnableGamepad2", "true") in ("1", "true", "yes"))
        enable_row.addRow("Enable Gamepad 1:", self._enable_p1)
        enable_row.addRow("Enable Gamepad 2:", self._enable_p2)
        vbox.addLayout(enable_row)

        for player, combos_list in [("Player 1", self._combos_p1), ("Player 2", self._combos_p2)]:
            key = "Controls" if player == "Player 1" else "ControlsP2"
            default = "DpadUp, DpadDown, DpadLeft, DpadRight, Back, Start, B, A, Y, X, Lb, Rb"
            controls_str = ini.get("GamepadMap", key, default)
            controls = [c.strip() for c in controls_str.split(",")]
            while len(controls) < 12:
                controls.append("")

            group = QGroupBox(f"SNES Button → Gamepad Button — {player}")
            ...
            for i, btn_name in enumerate(SNES_BUTTONS):
                ...
                combo.setCurrentText(controls[i] if i < len(controls) else "")
    ...
    def apply(self):
        self._ini.set("GamepadMap", "EnableGamepad1", "true" if self._enable_p1.isChecked() else "false")
        self._ini.set("GamepadMap", "EnableGamepad2", "true" if self._enable_p2.isChecked() else "false")
        self._ini.set("GamepadMap", "Controls",   ", ".join(c.currentText() for c in self._combos_p1))
        self._ini.set("GamepadMap", "ControlsP2", ", ".join(c.currentText() for c in self._combos_p2))
...
class ConfigWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Super Mario World Configuration")
        self.setMinimumSize(640, 560)

        if not INI_PATH.exists():
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(None, "Not installed", "Super Mario World is not installed yet.")
            return
